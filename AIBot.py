...
async def on_message(ctx: commands.Context, *args):
    query = ' '.join(args)
    print(query)
    try:
        # content = ctx.content[2:]
        # await ctx.channel.send(f"{gen(content)}")
        await ctx.channel.send(f"{gen(query)}")
    except:
        try:
            # content = ctx.content[3:]
            # await ctx.channel.send(f"{gen(content)}")
            await ctx.channel.send(f"{gen(query)}")
        except:
            await ctx.channel.send("일시오류! 다시 시도 해주세요!")
...
async def play(ctx: commands.Context, *args):
    ...
    with yt_dlp.YoutubeDL({'format': YTDL_FORMAT,
        ...
        await ctx.send('다운로드 중 ' + (f'https://youtu.be/{info["id"]}' if will_need_search else f'`{info["title"]}`'))
        try:
            ydl.download([query])
        except yt_dlp.utils.DownloadError as err:
            await notify_about_failure(ctx, err)
